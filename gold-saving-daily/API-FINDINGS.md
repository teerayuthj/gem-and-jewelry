...
| `GET /api/v1/gold-saving/monthly?month=YYYY-MM&amount=N` | สรุปเดือน — วันทำการ, ซื้อไปแล้วกี่วัน, ทองสะสม, `calculations` 4 ยอด และ `requested_plan` ของยอด N |
...
หน้าเว็บเลือก `effective_price || entry || latest_price` เพื่อแสดงการ์ดราคา และไม่อ่าน `reference_*`
ส่วนตารางแผนออมแสดง `calculations` และ `requested_plan` จาก `/monthly` โดยไม่คำนวณซ้ำที่ client
ระหว่าง rollout ถ้า API รุ่นเก่ายังไม่มี `requested_plan` component จะใช้ `initial_daily_amount`
กับ `latest_price.selling_price` เป็น fallback ชั่วคราว เพื่อไม่ให้แถวยอดของลูกค้าว่าง
...
- ใช้หัวคอลัมน์ "ทองที่ได้ต่อวัน (บาททอง)" และ "ทองที่ได้ทั้งเดือน (บาททอง)" เสมอ
  เพื่อระบุหน่วยน้ำหนักและไม่ผูกกับเวลาปัจจุบัน

สูตรคำนวณอยู่ที่ backend ฝั่ง Rust (`daily_allocations` / `calculate_plan`) ส่วน component
อ่าน `calculations` และ `requested_plan` จาก API มาแสดงเท่านั้น:

```
วันทำการ          = eligible_purchase_days (จันทร์–ศุกร์ ทั้งเดือน ไม่หักวันหยุดนักขัตฤกษ์)
ซื้อทองวันละ      = floor(ยอดออม × 100 / วันทำการ) / 100
ทองที่ซื้อได้ต่อวัน = truncate(ซื้อทองวันละ / ราคาขายออก, 6)
ทองที่ได้ทั้งเดือน  = truncate(ยอดออม / ราคาขายออก, 6)
```

น้ำหนักทองใช้กติกาธุรกิจตัดทศนิยมที่ 6 ตำแหน่ง (ไม่ปัดขึ้น) เช่นที่ราคาขายออก 68,100:
1,000 → 45.45 / 0.000667 / 0.014684, 2,000 → 90.90 / 0.001334 / 0.029368
และ 10,000 → 454.54 / 0.006674 / 0.146842
...
| 3 | เวลาเป็น string ไม่ใช่ ISO8601 / `saved_at` เป็น unix | `reference_time = "2026-09-18 08:31:20"` ไม่มี offset · `saved_at` ของ 17 ก.ย. เป็น 18 ก.ย. 08:13 (บันทึกย้อนหลัง) ทำให้แยก "เวลาราคา" กับ "เวลาบันทึก" ยาก | ส่ง ISO8601 `+07:00` และเพิ่ม `observed_at` แยกจาก `saved_at` |
| 4 | ไม่มี `Cache-Control` / `ETag` | ทุก request วิ่งถึง origin (header มีแต่ `x-cache-status`) | ก่อน cutoff `max-age=60` · หลัง settled `max-age=3600` + `ETag` |

ข้อ 1–2 คุ้มที่สุด และทำได้เลยโดยไม่ต้องรอข้อมูลสะสม
